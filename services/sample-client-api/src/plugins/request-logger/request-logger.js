...
var _RequestsLogger_instances,
    _RequestsLogger_producer,
    _RequestsLogger_acumulator,
    _RequestsLogger_topic,
    _RequestsLogger_mode,
    _RequestsLogger_key,
    _RequestsLogger_queueSize,
    _RequestsLogger_reconcilePropagate,
    _RequestsLogger_scheduleSend,
    _RequestsLogger_sendMessage
export default class RequestsLogger {
    ...
    async listen(fast, options, done) {
        ...
        fast.addHook('onSend', (request, reply, payload, done) => {
            ...
            const reqData = {
                key: __classPrivateFieldGet(this, _RequestsLogger_key, 'f'),
                body,
                query,
                params,
                rawHeaders,
            }
            __classPrivateFieldGet(this, _RequestsLogger_acumulator, 'f').push(
                reqData
            )
            if (
                __classPrivateFieldGet(this, _RequestsLogger_mode, 'f') ===
                `every`
            ) {
                __classPrivateFieldGet(
                    this,
                    _RequestsLogger_instances,
                    'm',
                    _RequestsLogger_sendMessage
                ).call(this, reqData)
            } else {
                __classPrivateFieldGet(
                    ...
                    this,
                    __classPrivateFieldGet(this, _RequestsLogger_queueSize, 'f')
                )
            }
            done()
        })
...
;(_RequestsLogger_producer = new WeakMap()),
    (_RequestsLogger_acumulator = new WeakMap()),
    (_RequestsLogger_topic = new WeakMap()),
    (_RequestsLogger_mode = new WeakMap()),
    (_RequestsLogger_key = new WeakMap()),
    (_RequestsLogger_queueSize = new WeakMap()),
    (_RequestsLogger_instances = new WeakSet()),
    (_RequestsLogger_reconcilePropagate =
        function _RequestsLogger_reconcilePropagate(queueSize) {
            if (
                __classPrivateFieldGet(this, _RequestsLogger_acumulator, 'f')
                    .length %
                    queueSize ===
                0
            ) {
    ...
    (_RequestsLogger_scheduleSend = function _RequestsLogger_scheduleSend() {}),
    (_RequestsLogger_sendMessage = async function _RequestsLogger_sendMessage(
        msg
    ) {
