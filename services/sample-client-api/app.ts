...
import configPlugin from './src/plugins/config/config-plugin.ts'
import kafkaPlugin from './src/plugins/kafka/kafka-plugin.ts'
import requestLoggerPlugin from './src/plugins/request-logger/request-logger-plugin.ts'
...
const server = async () => {
    ...
    fast.setErrorHandler((err, request, reply) => {
        ...
        reply.status(parseInt(code)).send({
            statusCode: code,
