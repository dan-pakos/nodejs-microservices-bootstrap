...
export default class KsqlDBClient {
    ...
    connect() {
        return new Promise((res, rej) => {
            __classPrivateFieldSet(
                this,
                _KsqlDBClient_client,
                http2.connect(
                    __classPrivateFieldGet(this, _KsqlDBClient_address, 'f')
                ),
                'f'
            )
            __classPrivateFieldGet(this, _KsqlDBClient_client, 'f').on(
                'error',
                (error) => rej(error)
            )
            __classPrivateFieldGet(this, _KsqlDBClient_client, 'f').on(
    ...
    request(query, cb = console.log) {
        return new Promise((res, rej) => {
            try {
                const session = __classPrivateFieldGet(
                    this,
                    _KsqlDBClient_client,
                    'f'
                ).request({
